...
设计目标: 低计算开销，适合 K1 RISC-V 边缘设备及 x86 服务器
"""
...
FOREIGN_PATTERNS = [
    r'\bqué\b', r'\bhace\b', r'\bleche\b', r'\bmañana\b',
    r'\bwhat\b', r'\bthe\b', r'\bthis\b', r'\bthat\b',
...
STRUCTURED_PATTERNS = [
    ...
    r'^-\s+\d+[\.\、]',
    r'^-\s+\*\*',
    r'简要分析', r'不可或缺', r'功能性.*美学',
    r'所有衣服中最核心', r'不同人群的需求',
    r'对美的追求和个人主义', r'个性化.*艺术感',
]
...
def check_hallucination(text: str) -> tuple[bool, int, list[str]]:
    ...
    for cat_name, patterns, weight, instant_trigger in categories:
        ...
        for pat in patterns:
            try:
                if re.search(pat, text_lower if pat.startswith(r'\b') or any(
                    c.isascii() and c.isalpha() for c in pat.replace(r'\b', '')
                ) else text):
                    cat_hits += 1
            except re.error:
                continue
